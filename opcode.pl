...
for ( @ops) {
    if (my $name = %alias{?$_}) {
	print \*STDOUT, "\tMEMBER_TO_FPTR($name),\t/* Perl_pp_$_ */\n";
    }
    else {
	print \*STDOUT, "\tMEMBER_TO_FPTR(Perl_pp_$_),\n";
    }
}
...
for ( @ops) {
    print \*STDOUT, "\t", tab(3, "MEMBER_TO_FPTR(Perl_%check{?$_}),"), "\t/* $_ */\n";
}
