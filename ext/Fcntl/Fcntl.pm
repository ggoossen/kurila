...
sub S_IFMT(@< @_)  { (nelems @_) ??  @( @_[0] ^&^ _S_IFMT() ) !! _S_IFMT()  }
sub S_IMODE(@< @_) { @_[0] ^&^ 07777 }

sub S_ISREG(@< @_)    { ( @_[0] ^&^ _S_IFMT() ) == S_IFREG()   }
sub S_ISDIR(@< @_)    { ( @_[0] ^&^ _S_IFMT() ) == S_IFDIR()   }
sub S_ISLNK(@< @_)    { ( @_[0] ^&^ _S_IFMT() ) == S_IFLNK()   }
sub S_ISSOCK(@< @_)   { ( @_[0] ^&^ _S_IFMT() ) == S_IFSOCK()  }
sub S_ISBLK(@< @_)    { ( @_[0] ^&^ _S_IFMT() ) == S_IFBLK()   }
sub S_ISCHR(@< @_)    { ( @_[0] ^&^ _S_IFMT() ) == S_IFCHR()   }
sub S_ISFIFO(@< @_)   { ( @_[0] ^&^ _S_IFMT() ) == S_IFIFO()   }
sub S_ISWHT(@< @_)    { ( @_[0] ^&^ _S_IFMT() ) == S_IFWHT()   }
sub S_ISENFMT(@< @_)  { ( @_[0] ^&^ _S_IFMT() ) == S_IFENFMT() }
