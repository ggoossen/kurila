...
$blessed_code = bless sub (@< @_) { "blessed" }, "Some::Package"
...
do
    my $safe = Safe->new()
    local $Storable::Eval = sub (@< @_) { $safe->reval(shift) }
...
do
    ...
    my $safe = MySafe->new()
    local $Storable::Eval = sub (@< @_) { $safe->reval(@_[0]) }
...
do
    ...
    my $short_sub = sub (@< @_) { "short sub" } # for SX_SCALAR
    # for SX_LSCALAR
