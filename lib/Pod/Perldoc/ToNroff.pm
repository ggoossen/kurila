...
sub center(@< @_)          { shift @_->_perldoc_elem('center'         , < @_) }
sub date(@< @_)            { shift @_->_perldoc_elem('date'           , < @_) }
sub fixed(@< @_)           { shift @_->_perldoc_elem('fixed'          , < @_) }
sub fixedbold(@< @_)       { shift @_->_perldoc_elem('fixedbold'      , < @_) }
sub fixeditalic(@< @_)     { shift @_->_perldoc_elem('fixeditalic'    , < @_) }
sub fixedbolditalic(@< @_) { shift @_->_perldoc_elem('fixedbolditalic', < @_) }
sub quotes(@< @_)          { shift @_->_perldoc_elem('quotes'         , < @_) }
sub release(@< @_)         { shift @_->_perldoc_elem('release'        , < @_) }
sub section(@< @_)         { shift @_->_perldoc_elem('section'        , < @_) }
