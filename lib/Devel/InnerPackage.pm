...
sub _loaded {
    ...
    no strict 'refs';
       ...
       return 1 if defined ${*{Symbol::fetch_glob("${name}::VERSION")}};
       return 1 if defined @{*{Symbol::fetch_glob("${name}::ISA")}};

       # Are there any symbol table entries other than other namespaces
       foreach ( keys %{*{Symbol::fetch_glob("${name}::")}} ) {
               next if substr($_, -2, 2) eq '::';
               return 1 if defined &{Symbol::fetch_glob("${name}::$_")};
       }
