...
$_ = $stdtext3;
expect	\@( extract_multiple(undef, \@( sub { m/(\G[a-z])/gc && $1 } )) ),
...
$_ = $stdtext3;
expect	\@( scalar extract_multiple(undef, \@( sub { m/(\G[a-z])/gc && $1 } )) ),
	\@( divide($stdtext3 => 1) );
