...
AUTOLOAD {
    ...
    $@ = $save;
    defined(&{Symbol::fetch_glob($AUTOLOAD)}) || die "SelfLoader inconsistency error";
...
sub _load_stubs {
    ...
    no strict "refs";
    my $fh = \*{Symbol::fetch_glob("${callpack}::DATA")};
    use strict;
	...
	if ($line =~ m/^sub\s+([\w:]+)\s*((?:\([\\\$\@\%\&\*\;]*\))?(?:$attr_list)?)/) {
            ...
            if (index($1,'::') == -1) {         # simple sub name
                ...
                $name = $1;
                $name =~ m/^(.*)::/;
                if (defined(&{Symbol::fetch_glob("${1}::AUTOLOAD")})) {
                    \&{Symbol::fetch_glob("${1}::AUTOLOAD")} == \&SelfLoader::AUTOLOAD ||
