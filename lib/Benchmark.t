...
isa_ok: (timeit: 5, sub (@< @_) {++$foo}), 'Benchmark', "timeit CODEREF"
...
my $coderef = sub (@< @_) {$baz += (fib: $ballast)}
...
$foo = 0
my $got = do_with_out:  { (timethis: $iterations, sub (@< @_) {++$foo}) } 
...
$foo = 0
$out = ""
$got = do_with_out: { (timethis: $iterations, sub (@< @_) {++$foo}, $title) }
...
$foo = 0
$out = ""
$got = do_with_out: { (timethis: $iterations, sub (@< @_) {++$foo}, $title, 'nop') }
...
do
    ...
    $out = ""
    my $start = time
    $got = do_with_out: { (timethis: -2, sub (@< @_) {$foo+= (fib: $ballast)}, $title, 'none') }
    my $end = time
