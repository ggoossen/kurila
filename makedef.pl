...
while (@ARGV)
    ...
    if ($flag =~ s/^CC_FLAGS=/ /)
        for my $fflag ($flag =~ m/(?:^|\s)-D(\S+)/g)
            ...
            %define{+$1} = $2   if $fflag =~ m/^(\w+)=(.+)$/
        next

    %define{+$1} = 1 if ($flag =~ m/^-D(\w+)$/)
    %define{+$1} = $2 if ($flag =~ m/^-D(\w+)=(.+)$/)
    $CCTYPE   = $1 if ($flag =~ m/^CCTYPE=(\w+)$/)
    $PLATFORM = $1 if ($flag =~ m/^PLATFORM=(\w+)$/)
    if ($PLATFORM eq 'netware')
        $FILETYPE = $1 if ($flag =~ m/^FILETYPE=(\w+)$/)
...
if ($PLATFORM eq 'aix')
    1 # Nothing for now.
elsif ($PLATFORM =~ m/^win(?:32|ce)$/ || $PLATFORM eq 'netware')
...
unless ($PLATFORM eq 'win32' || $PLATFORM eq 'wince' || $PLATFORM eq 'MacOS' || $PLATFORM eq 'netware')
    ...
    while ( ~< $cfg_fh)
        if (m/^(?:ccflags|optimize)='(.+)'$/)
            $_ = $1
            %define{$1} = 1 while m/-D(\w+)/g
        if (m/^(d_(?:mmap|sigaction))='(.+)'$/)
            %define{$1} = $2
        if ($PLATFORM eq 'os2')
...
sub skip_symbols($list)
    foreach my $symbol ($list)
        %skip{+$symbol} = 1

sub emit_symbols($list)
    foreach my $symbol ($list)
        my $skipsym = $symbol
        # XXX hack
...
#           print $^STDOUT, "\t_$symbol\n";
#           print $^STDOUT, "\t$symbol = _$symbol\n";
#       elsif ($CCTYPE eq 'GCC') {
