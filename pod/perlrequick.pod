...
In this statement, C<World> is a regex and the C<m//> enclosing
C<m/World/> tells perl to search a string for a match.  The operator
...
Finally, the C<m//> default delimiters for a match can be changed to
...
statement to be true:
    ...
    "Hello World" =~ m/world/;  # doesn't match, case sensitive
    "Hello World" =~ m/o W/;    # matches, ' ' is an ordinary char
    "Hello World" =~ m/World /; # doesn't match, no ' ' at end
...
A metacharacter can be matched by putting a backslash before it:
    ...
    "/usr/bin/perl" =~ m/\/usr\/bin\/perl/;  # matches
...
C<$> means match at the end of the string, or before a newline at the
    ...
    "housekeeper" =~ m/keeper$/;        # matches
    "housekeeper\n" =~ m/keeper$/;      # matches
    "housekeeper" =~ m/^housekeeper$/;  # matches
...
the class, the earliest point at which the regex can match is C<'a'>.
    ...
    m/[yY][eE][sS]/; # match 'yes' in a case-insensitive way
                    # 'yes', 'Yes', 'YES', etc.
    m/yes/i;         # also match 'yes' in a case-insensitive way
...
They can be used just as ordinary variables:
    ...
    $time =~ m/(\d\d):(\d\d):(\d\d)/;  # match hh:mm:ss format
    $hours = $1;
    $minutes = $2;
    $seconds = $3;

In list context, a match C<m/regex/> with groupings will return the
...
matching variables that can be used I<inside> a regex:
    ...
    m/(\w\w\w)\s\1/; # find sequences like 'the the' in string
...
Here are some examples:
    ...
    m/[a-z]+\s+\d*/;  # match a lowercase word, at least some space, and
...
C<$pattern> won't be changing, use the C<m//o> modifier, to only
...
In the replacement expression C<{...}> can be used to evaluate code.
Some examples:

    # reverse all the words in a string
    $x = "the cat in the hat";
    $x =~ s/(\w+)/{ reverse $1 }/g;   # $x contains "eht tac ni eht tah"

    # convert percentage to decimal
    $x = "A 39% hit rate";
    $x =~ s!(\d+)%!{ $1/100 }!;       # $x contains "A 0.39 hit rate"

The last example shows that C<s///> can use other delimiters, such as
C<s!!!> and C<s{}{}>, and even C<s{}//>..

=head2 The split operator

C<split m/regex/, string> splits C<string> into a list of substrings
...
string into words, use
    ...
    $x = "Calvin and Hobbes";
    @word = split m/\s+/, $x;  # $word[0] = 'Calvin'
                              # $word[1] = 'and'
                              # $word[2] = 'Hobbes'
...
To extract a comma-delimited list of numbers, use
    ...
    $x = "1.618,2.718,   3.142";
    @const = split m/,\s*/, $x;  # $const[0] = '1.618'
                                # $const[1] = '2.718'
                                # $const[2] = '3.142'

If the empty regex C<m//> is used, the string is split into individual
