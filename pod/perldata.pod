...
be a chain of identifiers, separated by C<::>; all but the last are
interpreted as names of packages, to locate the namespace in which to
look up the final identifier (see L<perlmod/Packages> for details).
It's possible to substitute for a simple identifier, an expression
that produces a reference to the value at runtime.  This is described
in more detail below and in L<perlref>.  X<identifier>
...
Scalar variables are always named with '$'. And they variables and do
not change depending on the value being refered as is done in Perl 5:
X<scalar>

    $days		# the simple scalar variable "days"
...
which return whatever they feel like returning.) For examle:
X<array, length>

    $element_count = @whatever
...
=head3 Version syntax
X<version string> X<version syntax> X<vstring> X<v-string>

A literal of the form C<v1.20.300.4000> creates a version object,
as returned by C<version->new>.
...
An array is subscripted the name of the array (including leading
C<@>b), then the subscript inside square brackets.  For example:
