...
Usage of a string to lookup a symbol, like for example ${"$class\::VERSION"} will always give an error,
regardless of "use strict 'refs'".
You can use C<Symbol::qualify_to_ref> to lookup a symbol.
Symbol::qualify_to_ref retuns a reference to a glob, thus to use the glob you need to explicitly dereference it.
For example to get the above ${"$class\::VERSION"} you need C<${*{Symbol::qualify_to_ref("$class\::VERSION")}}>
Symbol::qualify_to_ref is defined in the Perl core and does not require an C<use Symbol>.

=head2 The <> operator no longer supports glob patterns.

The <'pattern'> glob operator, has been removed in favor of the "glob"
...
bleadperl changes upto 31540 are integrated
...
Documention has not been updated for many of the changes for kurila.
