...
correct ways to read the whole file at once:
    ...
    open my $fh, "<", "foo" or die $^OS_ERROR;
...
The pattern matching operations C<m//> and C<s///>
...
The string matched by the last successful pattern match of a pattern
compiled or executed with the C</p> modifier (not counting any matches
hidden within a BLOCK or eval() enclosed by the current BLOCK).  This
...
pattern match of a pattern compiled or executed with the C</p>
modifier (not counting any matches hidden within a BLOCK or eval()
enclosed by the current BLOCK). This variable is read-only and
...
line boundaries when quoting poetry.)
    ...
    local $^INPUT_RECORD_SEPARATOR;           # enable "slurp" mode
    local $_ = <FH>;    # whole file now here
    s/\n[ \t]+/ /g;

Remember: the value of C<$^INPUT_RECORD_SEPARATOR> is a string, not a regex.  B<awk> has to be
better for something. :-)

Setting C<$^INPUT_RECORD_SEPARATOR> to a reference to an integer, scalar containing an integer, or
scalar that's convertible to an integer will attempt to read records
instead of lines, with the maximum record size being the referenced
integer.  So this:

    local $^INPUT_RECORD_SEPARATOR = \32768; # or \"32768", or \$var_containing_32768
    open my $fh, "<", $myfile or die $^OS_ERROR;
    local $_ = <$fh>;
...
system or not; C<$^OUTPUT_AUTOFLUSH> tells you only whether you've asked Perl
...
(Mnemonic: you set C<$^OUTPUT_RECORD_SEPARATOR> instead of adding "\n" at the end of the print.
Also, it's just like C<$^INPUT_RECORD_SEPARATOR>, but it's what you get "back" from Perl.)
...
exit value of the subprocess is really (C<<< $^CHILD_ERROR >> 8 >>>), and
C<$^CHILD_ERROR ^&^ 127> gives which signal, if any, the process died from, and
C<$^CHILD_ERROR ^&^ 128> reports whether there was a core dump.  (Mnemonic:
similar to B<sh> and B<ksh>.)

Additionally, if the C<h_errno> variable is supported in C, its value
is returned via $^CHILD_ERROR if any C<gethost*()> function fails.

If you have installed a signal handler for C<SIGCHLD>, the
value of C<$^CHILD_ERROR> will usually be wrong outside that handler.

Inside an C<END> subroutine C<$^CHILD_ERROR> contains the value that is going to be
given to C<exit()>.  You can modify C<$^CHILD_ERROR> in an C<END> subroutine to
change the exit status of your program.  For example:

    END {
	$^CHILD_ERROR = 1 if $^CHILD_ERROR == 255;  # die would make it 255
    } 

Under VMS, the pragma C<use vmsish 'status'> makes C<$^CHILD_ERROR> reflect the
actual VMS exit status, instead of the default emulation of POSIX
status; see L<perlvms/$^CHILD_ERROR> for details.
...
as $^CHILD_ERROR when the pragma C<use vmsish 'status'> is in effect.
...
=item $^OS_ERROR
X<$^OS_ERROR> X<$OS_ERROR>

If used numerically, yields the current value of the C C<errno>
variable, or in other words, if a system or library call fails, it
sets this variable.  This means that the value of C<$^OS_ERROR> is meaningful
only I<immediately> after a B<failure>:

    if (open my $fh, "<", $filename) {
	# Here $^OS_ERROR is meaningless.
	...
    } else {
	# ONLY here is $^OS_ERROR meaningful.
	...
	# Already here $^OS_ERROR might be meaningless.
    }
...
You can assign a number to C<$^OS_ERROR> to set I<errno> if, for instance,
you want C<"$^OS_ERROR"> to return the string for error I<n>, or you want
...
the moment, this differs from C<$^OS_ERROR> under only VMS, OS/2, and Win32
(and for MacPerl).  On all other platforms, C<$^E> is always just
the same as C<$^OS_ERROR>.

Under VMS, C<$^E> provides the VMS status value from the last
system error.  This is more specific information about the last
system error than that provided by C<$^OS_ERROR>.  This is particularly
important when C<$^OS_ERROR> is set to B<EVMSERR>.
...
Caveats mentioned in the description of C<$^OS_ERROR> generally apply to
...
If $^EVAL_ERROR is the null string, the last eval() parsed and executed
...
be portable, this behavior is not reflected by C<$^PID>, whose value remains
...
=item $^UID
X<$^UID> X<$REAL_USER_ID>

The real uid of this process.  (Mnemonic: it's the uid you came I<from>,
if you're running setuid.)  You can change both the real uid and
the effective uid at the same time by using POSIX::setuid().  Since
changes to $^UID require a system call, check $^OS_ERROR after a change attempt to 
detect any possible errors.

=item $EFFECTIVE_USER_ID

=item $^EUID
X<$^EUID> X<$EFFECTIVE_USER_ID>

The effective uid of this process.  Example:

    $^UID = $^EUID;		# set real to effective uid
    @($^UID,$^EUID) = @($^EUID,$^UID);	# swap real and effective uid

You can change both the effective uid and the real uid at the same
time by using POSIX::setuid().  Changes to $^EUID require a check to $^OS_ERROR
to detect any possible errors after an attempted change. 

(Mnemonic: it's the uid you went I<to>, if you're running setuid.)
C<< $^UID >> and C<< $^EUID >> can be swapped only on machines
supporting setreuid().
...
However, a value assigned to C<$^GID> must be a single number used to
set the real gid.  So the value given by C<$^GID> should I<not> be assigned
back to C<$^GID> without being forced numeric, such as by adding zero. Note
that this is different to the effective gid (C<$^EGID>) which does take a
list.

You can change both the real gid and the effective gid at the same
time by using POSIX::setgid().  Changes to $^GID require a check to $^OS_ERROR
to detect any possible errors after an attempted change.
...
Similarly, a value assigned to C<$^EGID> must also be a space-separated
...
Changes to $) require a check to $^OS_ERROR to detect any possible errors
after an attempted change.

(Mnemonic: parentheses are used to I<group> things.  The effective gid
is the group that's I<right> for you, if you're running setgid.)

C<< $< >>, C<< $> >>, C<$^GID> and C<$^EGID> can be set only on
machines that support the corresponding I<set[re][ug]id()> routine.  C<$^GID>
and C<$^EGID> can be swapped only on machines supporting setregid().
...
The routine indicated by C<$^WARN_HOOK> is called when a warning message is
about to be printed.  The warning message is passed as the first
argument.  The default $^WARN_HOOK calls C<message> on the object and print it
to C<STDERR>.
...
To save warnings:
    ...
    local $^WARN_HOOK = sub { push @warn, @_[0] };

=item $^DIE_HOOK

The routine indicated by C<$^DIE_HOOK> is called when a fatal exception
is about to be thrown.  The error message is passed as the first
argument. The <$^DIE_HOOK> is disabled inside an eval().
...
The array $^INCLUDE_PATH contains the list of places that the C<do EXPR>,
...
code directly into $^INCLUDE_PATH.  Those hooks may be subroutine references, array
...
The hash $^INCLUDED contains entries for each filename included via the
...
by default inserted into $^INCLUDED in place of a filename.  Note, however,
that the hook may have set the $^INCLUDED entry by itself to provide some more
specific info.
...
The variables C<$^EVAL_ERROR>, C<$^OS_ERROR>, C<$^E>, and C<$^CHILD_ERROR> contain information
...
following Perl expression, which uses a single-quoted string:
    ...
    eval q{
	open my $pipe, "/cdrom/install |" or die $^OS_ERROR;
	my @res = <$pipe>;
	close $pipe or die "bad pipe: $^CHILD_ERROR, $^OS_ERROR";
    };

After execution of this statement all 4 variables may have been set.  

C<$^EVAL_ERROR> is set if the string to be C<eval>-ed did not compile (this
...
the value of $^EVAL_ERROR is the compile error, or the argument to C<die>
(which will interpolate C<$^OS_ERROR> and C<$^CHILD_ERROR>).  (See also L<Fatal>,
though.)

When the eval() expression above is executed, open(), C<< <PIPE> >>,
and C<close> are translated to calls in the C run-time library and
thence to the operating system kernel.  C<$^OS_ERROR> is set to the C library's
...
Finally, C<$^CHILD_ERROR> may be set to non-0 value if the external program
...
contrast to C<$^OS_ERROR> and C<$^E>, which are set only if error condition
is detected, the variable C<$^CHILD_ERROR> is set on each C<wait> or pipe
C<close>, overwriting the old value.  This is more like C<$^EVAL_ERROR>, which
on every eval() is always set on failure and cleared on success.

For more details, see the individual descriptions at C<$^EVAL_ERROR>, C<$^OS_ERROR>, C<$^E>,
and C<$^CHILD_ERROR>.
...
C<::>.  In this case, the part before the last C<::>  is taken
to be a I<package qualifier>; see L<perlmod>.

Perl variable names may also be a sequence of digits or a single
punctuation or a name starting with C<^>.  These names are all reserved for
special uses by Perl; for example, the all-digits names are used
to hold data captured by backreferences after a regular expression
match.

Perl variable names may be alphanumeric
strings that begin with a caret.
These variables are
...
handlers is simply wrong.  C<$DIE_HOOK> as currently implemented
