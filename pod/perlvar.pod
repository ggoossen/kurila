...
integer.  So this:
    ...
    open my $fh, "<", $myfile or die $^ERRNO;
    local $_ = <$fh>;
...
sets this variable.  This means that the value of C<$^ERRNO> is meaningful
only I<immediately> after a B<failure>:

    if (open my $fh, "<", $filename) {
	# Here $^ERRNO is meaningless.
	...
    } else {
	# ONLY here is $^ERRNO meaningful.
	...
	# Already here $^ERRNO might be meaningless.
    }
...
You can assign a number to C<$^ERRNO> to set I<errno> if, for instance,
you want C<"$^ERRNO"> to return the string for error I<n>, or you want
...
the moment, this differs from C<$^ERRNO> under only VMS, OS/2, and Win32
...
system error than that provided by C<$^ERRNO>.  This is particularly
important when C<$^ERRNO> is set to B<EVMSERR>.
...
Caveats mentioned in the description of C<$^ERRNO> generally apply to
...
If $^EVALERROR is the null string, the last eval() parsed and executed
...
changes to $^UID require a system call, check $^ERRNO after a change attempt to 
...
time by using POSIX::setuid().  Changes to $^EUID require a check to $^ERRNO
...
time by using POSIX::setgid().  Changes to $^GID require a check to $^ERRNO
to detect any possible errors after an attempted change.
...
Changes to $) require a check to $^ERRNO to detect any possible errors
...
The variables C<$^EVALERROR>, C<$^ERRNO>, C<$^E>, and C<$^CHILD_ERROR> contain information
...
following Perl expression, which uses a single-quoted string:
    ...
    eval q{
	open my $pipe, "/cdrom/install |" or die $^ERRNO;
	my @res = <$pipe>;
	close $pipe or die "bad pipe: $^CHILD_ERROR, $^ERRNO";
    };

After execution of this statement all 4 variables may have been set.  

C<$^EVALERROR> is set if the string to be C<eval>-ed did not compile (this
...
the value of $^EVALERROR is the compile error, or the argument to C<die>
(which will interpolate C<$^ERRNO> and C<$^CHILD_ERROR>).  (See also L<Fatal>,
though.)

When the eval() expression above is executed, open(), C<< <PIPE> >>,
and C<close> are translated to calls in the C run-time library and
thence to the operating system kernel.  C<$^ERRNO> is set to the C library's
...
contrast to C<$^ERRNO> and C<$^E>, which are set only if error condition
is detected, the variable C<$^CHILD_ERROR> is set on each C<wait> or pipe
C<close>, overwriting the old value.  This is more like C<$^EVALERROR>, which
on every eval() is always set on failure and cleared on success.

For more details, see the individual descriptions at C<$^EVALERROR>, C<$^ERRNO>, C<$^E>,
and C<$^CHILD_ERROR>.
