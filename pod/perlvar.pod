...
C<substr $_, @-[@- -1], @+[@- -1] - @-[@- -1]>.  One can use C<@- -1> to find the last
matched subgroup in the last successful match.  Contrast with
C<@+ -1>, the number of subgroups in the regular expression.  Compare
with C<@+>.
...
=item C<$^PREMATCH> is the same as C<substr($var, 0, @-[0])>

=item C<$^MATCH> is the same as C<substr($var, @-[0], @+[0] - @-[0])>

=item C<$^POSTMATCH> is the same as C<substr($var, @+[0])>
...
The variables C<$@>, C<$^ERRNO>, C<$^E>, and C<$^CHILD_ERROR> contain information
...
For more details, see the individual descriptions at C<$@>, C<$^ERRNO>, C<$^E>,
and C<$^CHILD_ERROR>.
