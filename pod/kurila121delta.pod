...
kurila121delta - what is new for Perl Kurila 1.20
...
=head2 Layout based syntax for arrays and hashes.

=head2 Subroutines are first order types.

=head1 Core Enhancements

=head2 Layout for arrays and hashes

Layout can be used to make arrays and hasesh.
...
=head2 CODE is a first order data type

C<&foo> returns the sub foo, i.e. its type is CODE.
Codes can be copied, and modified.
...
=head2 Subcalls save to sub to be called at compile time

Implies that to change the sub to be called after code is compile the sub
shuold be modified, for example by C<< *foo->& = sub { "new" } >>.
...
=head2 Annonymous arrays and hashes constructors are C< @: > and C< %: >

@: and %: which use the layout based syntax.

=head3 Rationale

Move everything towards layout based syntax.
