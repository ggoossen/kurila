...
it is a NULL pointer, then C<obj> is merely stored, without the
...
In Perl Kurila all strings are assumed to be UTF-8. If you want to access
it like a sequence of codepoints you should assume UTF-8 encoding.

In Perl Kurila there is no SVf_UTF8 flag and you can do things like:
...
You just have a sequence of bytes, you can do with it what you want.

=item *

When you want to deal whith codepoints (characters), strings are
default assumed UTF-8 encoded.

=item *

When writing a codepoint C<uv> to a string, B<always> use
C<uv_to_utf8>, unless C<UTF8_IS_INVARIANT(uv))> in which case
you can use C<*s = uv>.

=item *

There are no UTF-8 and non-UTF-8 strings. You can combine all strings
easily, of course when one of the string is not valid UTF-8, then also
the result will be not valid UTF-8.
