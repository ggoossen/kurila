...
Run-time customization of $^INCLUDE_PATH can be enabled by passing the
...
then be set up to add additional entries to $^INCLUDE_PATH.

There is alpha support for relocatable $^INCLUDE_PATH entries.
