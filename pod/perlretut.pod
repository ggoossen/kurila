...
C<m//> enclosing C<m/World/> tells Perl to search a string for a match.
...
And finally, the C<m//> default delimiters for a match can be changed
...
Let's consider how different regexps would match C<"Hello World">:
    ...
    "Hello World" =~ m/world/;  # doesn't match
    "Hello World" =~ m/o W/;    # matches
    "Hello World" =~ m/oW/;     # doesn't match
    "Hello World" =~ m/World /; # doesn't match
...
always match at the earliest possible point in the string:
    ...
    "Hello World" =~ m/o/;       # matches 'o' in 'Hello'
    "That hat is red" =~ m/hat/; # matches 'hat' in 'That'
...
that a metacharacter can be matched by putting a backslash before it:
    ...
    "2+2=4" =~ m/2+2/;    # doesn't match, + is a metacharacter
    "2+2=4" =~ m/2\+2/;   # matches, \+ is treated like an ordinary +
    "The interval is [0,1)." =~ m/[0,1)./     # is a syntax error!
    "The interval is [0,1)." =~ m/\[0,1\)\./  # matches
...
bytes.  Here are some examples of escapes:
    ...
    "1000\n2000" =~ m/0\n20/   # matches
    "1000\t2000" =~ m/\000\t2/ # doesn't match, "0" ne "\000"
    "cat"        =~ m/\143\x61\x74/ # matches, but a weird way to spell cat
...
evaluated for matching purposes.  So we have:
    ...
    $foo = 'house';
    'housecat' =~ m/$foo/;      # matches
    'cathouse' =~ m/cat$foo/;   # matches
    'housecat' =~ m/${foo}cat/; # matches
...
all the files.  For each line, S<C<print if m/$regexp/;>> prints the
line if the regexp matches the line.  In this line, both C<print> and
C<m/$regexp/> use the default variable C<$_> implicitly.
...
C<$> means match at the end of the string, or before a newline at the
    ...
    "housekeeper" =~ m/keeper/;    # matches
    "housekeeper" =~ m/^keeper/;   # doesn't match
    "housekeeper" =~ m/keeper$/;   # matches
    "housekeeper\n" =~ m/keeper$/; # matches
...
matches the whole string.  Consider
    ...
    "keeper" =~ m/^keep$/;      # doesn't match
    "keeper" =~ m/^keeper$/;    # matches
    ""       =~ m/^$/;          # ^$ matches an empty string
...
bert, off in a string by himself:
    ...
    "dogbert" =~ m/bert/;   # matches, but not what you want

    "dilbert" =~ m/^bert/;  # doesn't match, but ..
    "bertram" =~ m/^bert/;  # matches, so still not good enough

    "bertram" =~ m/^bert$/; # doesn't match, good
    "dilbert" =~ m/^bert$/; # doesn't match, good
    "bert"    =~ m/^bert$/; # matches, perfect
...
to be possibly matched inside.  Here are some examples:
    ...
    m/cat/;       # matches 'cat'
    m/[bcr]at/;   # matches 'bat, 'cat', or 'rat'
    m/item[0123456789]/;  # matches 'item0' or ... or 'item9'
    "abc" =~ m/[cab]/;    # matches 'a'
...
string is the earliest point at which the regexp can match.
    ...
    m/[yY][eE][sS]/;      # match 'yes' in a case-insensitive way
                         # 'yes', 'Yes', 'YES', etc.
...
appending an C<'i'> to the end of the match.  Then C<m/[yY][eE][sS]/;>
can be rewritten as C<m/yes/i;>.  The C<'i'> stands for
...
special characters C<]$\> are handled:
   ...
   m/[\]c]def/; # matches ']def' or 'cdef'
   $x = 'bcr';
   m/[$x]at/;   # matches 'bat', 'cat', or 'rat'
   m/[\$x]at/;  # matches '$at' or 'xat'
   m/[\\$x]at/; # matches '\at', 'bat, 'cat', or 'rat'
...
become the svelte C<[0-9]> and C<[a-z]>.  Some examples are
    ...
    m/item[0-9]/;  # matches 'item0' or ... or 'item9'
    m/[0-9bx-z]aa/;  # matches '0aa', ..., '9aa',
                    # 'baa', 'xaa', 'yaa', or 'zaa'
    m/[0-9a-fA-F]/;  # matches a hexadecimal digit
    m/[0-9a-zA-Z_]/; # matches a "word" character,
...
character, or the match fails.  Then
    ...
    m/[^a]at/;  # doesn't match 'aat' or 'at', but matches
               # all other 'bat', 'cat, '0at', '%at', etc.
    m/[^0-9]/;  # matches a non-numeric character
    m/[a^]at/;  # matches 'aat' or '^at'; here '^' is ordinary
...
of character classes.  Here are some in use:
    ...
    m/\d\d:\d\d:\d\d/; # matches a hh:mm:ss time format
    m/[\d\s]/;         # matches any digit or whitespace character
    m/\w\W\w/;         # matches a word char, followed by a
                      # non-word char, followed by a word char
    m/..rt/;           # matches any two chars, followed by 'rt'
    m/end\./;          # matches 'end.'
    m/end[.]/;         # same thing, matches 'end.'
...
character C<\w\W> or C<\W\w>:
    ...
    $x =~ m/cat/;    # matches cat in 'housecat'
    $x =~ m/\bcat/;  # matches cat in 'catenates'
    $x =~ m/cat\b/;  # matches cat in 'housecat'
    $x =~ m/\bcat\b/;  # matches 'cat' at end of string
...
of it as empty.  Then
    ...
    ""   =~ m/^$/;    # matches
    "\n" =~ m/^$/;    # matches, $ anchors before "\n"

    ""   =~ m/./;      # doesn't match; it needs a char
    ""   =~ m/^.$/;    # doesn't match; it needs a char
    "\n" =~ m/^.$/;    # doesn't match; it needs a char other than "\n"
    "a"  =~ m/^.$/;    # matches
    "a\n"  =~ m/^.$/;  # matches, $ anchors before "\n"
...
no modifiers (m//): Default behavior.  C<'.'> matches any character
...
Here are examples of C<//s> and C<//m> in action:
    ...
    $x =~ m/^Who/;   # doesn't match, "Who" not at start of string
    $x =~ m/^Who/s;  # doesn't match, "Who" not at start of string
    $x =~ m/^Who/m;  # matches, "Who" at start of second line
    $x =~ m/^Who/sm; # matches, "Who" at start of second line

    $x =~ m/girl.Who/;   # doesn't match, "." doesn't match "\n"
    $x =~ m/girl.Who/s;  # matches, "." matches "\n"
    $x =~ m/girl.Who/m;  # doesn't match, "." doesn't match "\n"
    $x =~ m/girl.Who/sm; # matches, "." matches "\n"
...
the newline before, like C<$>), and C<\z> (matches only the end):
    ...
    $x =~ m/^Who/m;   # matches, "Who" at start of second line
    $x =~ m/\AWho/m;  # doesn't match, "Who" is not at start of string

    $x =~ m/girl$/m;  # matches, "girl" at end of first line
    $x =~ m/girl\Z/m; # doesn't match, "girl" is not at end of string

    $x =~ m/Perl\Z/m; # matches, "Perl" is at newline before end
    $x =~ m/Perl\z/m; # doesn't match, "Perl" is not at end of string
...
examples:
    ...
    "cats and dogs" =~ m/cat|dog|bird/;  # matches "cat"
    "cats and dogs" =~ m/dog|cat|bird/;  # matches "cat"

Even though C<dog> is the first alternative in the second regexp,
C<cat> is able to match earlier in the string.

    "cats"          =~ m/c|ca|cat|cats/; # matches "c"
    "cats"          =~ m/cats|cat|ca|c/; # matches "cats"
...
are
    ...
    m/(a|b)b/;    # matches 'ab' or 'bb'
    m/(ac|b)b/;   # matches 'acb' or 'bb'
    m/(^a|b)c/;   # matches 'ac' at start of string or 'bc' anywhere
    m/(a|[bc])d/; # matches 'ad', 'bd', or 'cd'

    m/house(cat|)/;  # matches either 'housecat' or 'house'
    m/house(cat(s|)|)/;  # matches either 'housecats' or 'housecat' or
                        # 'house'.  Note groups can be nested.

    m/(19|20|)\d\d/;  # match years 19xx, 20xx, or the Y2K problem, xx
    "20" =~ m/(19|20|)\d\d/;  # matches the null alternative '()\d\d',
...
up and declare S<C<$string =~ m/(abd|abc)(df|d|de)/;>> to be false.
...
tempted to use it as a part of some other pattern:
    ...
    $line = "code=e99e";
    if ($line =~ m/^(\w+)=$a99a$/){   # unexpected behavior!
        print "$1 is valid\n";
    } else {
        print "bad line: '$line'\n";
    }
...
previous pattern:
    ...
    if ( $time =~ m/(?|(\d\d|\d):(\d\d)|(\d\d)(\d\d))\s+([A-Z][A-Z][A-Z])/ ){
...
this code
    ...
    $x =~ m/^(Mmm|Yech)\.\.\.(donut|peas)/; # matches
    foreach $expr (1..$#-) {
...
of the string after the match.  An example:
    ...
    $x =~ m/cat/;  # $` = 'the ', $& = 'cat', $' = ' caught the mouse'
    $x =~ m/the/;  # $` = '', $& = 'the', $' = ' cat caught the mouse'
...
required for some reason:
    ...
    $x = '12aba34ba5';
    @num = split m/(a|b)+/, $x;    # @num = ('12','a','34','b','5')
    @num = split m/(?:a|b)+/, $x;  # @num = ('12','34','5')
...
Here are some examples:
    ...
    m/[a-z]+\s+\d*/;  # match a lowercase word, at least one space, and
                     # any number of digits
    m/(\w+)\s+\1/;    # match doubled words of arbitrary length
    m/y(es)?/i;       # matches 'y', 'Y', or a case-insensitive 'yes'
    $year =~ m/\d{2,4}/;  # make sure year is at least 2 but not more
                         # than 4 digits
    $year =~ m/\d{4}|\d{2}/;    # better match; throw out 3 digit dates
    $year =~ m/\d{2}(\d{2})?/;  # same thing written differently. However,
...
with C<m/a?.../>, Perl will first try to match the regexp with the C<a>
...
These can be combined into a single regexp with a three-way alternation:
   ...
   m/[+-]?(\d+\.\d+|\d+\.|\.\d+)/;  # floating point, no exponent
...
mantissa.  The overall form of the regexp now becomes clear:
    ...
    m/^(optional sign)(integer | f.p. mantissa)(optional exponent)$/;
...
C<m/regexp/> and arbitrary delimiter C<m!regexp!> forms.  We have used
...
(or compile) C<$pattern> each time through the loop.
...
the regexp in the I<last successful match> is used instead.  So we have
    ...
    "dog" =~ m/d/;  # 'd' matches
    "dogbert =~ m//;  # this matches the 'd' regexp used before
...
off.  C<\G> allows us to easily do context-sensitive matching:
    ...
    $x =~ m/^([+-]?\d+)\s*/g;  # get magnitude
    $weight = $1;
    if ($metric) { # error checking
        print "Units error!" unless $x =~ m/\Gkg\./g;
    }
    else {
        print "Units error!" unless $x =~ m/\Glbs\./g;
    }
    $x =~ m/\G\s+(widget|sprocket)/g;  # continue processing
...
S<C<GTT GAA>> gives a match.  A better solution is
    ...
    while ($dna =~ m/(\w\w\w)*?TGA/g) {  # note the minimal *?
...
occurrences of the regexp on each line.  As with C<simple_grep>, both the
...
number of strings.  For example, to split a string into words, use
    ...
    $x = "Calvin and Hobbes";
    @words = split m/\s+/, $x;  # $word[0] = 'Calvin'
                               # $word[1] = 'and'
                               # $word[2] = 'Hobbes'

If the empty regexp C<m//> is used, the regexp always matches and
...
upper case, respectively:
    ...
    $x = "perl";
    $string =~ m/\u$x/;  # matches 'Perl' in $string
    $x = "M(rs?|s)\\."; # note the double backslash
    $string =~ m/\l$x/;  # matches 'mr.', 'mrs.', and 'ms.',
...
terminated by C<\E> or thrown over by another C<\U> or C<\L>:
    ...
    $x =~ m/\Ukeypunch/;  # matches punch card string
...
instance,
    ...
    $x = "\QThat !^*&%~& cat!";
    $x =~ m/\Q!^*&%~&\E/;  # check for rough language
...
than 255.
    ...
    m/\x{263a}/;  # match a Unicode smiley face :)
...
example, to match lower and uppercase characters,
    ...
    use charnames ":full"; # use named chars with Unicode full names
    $x = "BOB";
    $x =~ m/^\p{IsUpper}/;   # matches, uppercase char class
    $x =~ m/^\P{IsUpper}/;   # doesn't match, char class sans uppercase
    $x =~ m/^\p{IsLower}/;   # doesn't match, lowercase char class
    $x =~ m/^\P{IsLower}/;   # matches, char class sans lowercase
...
traditional Unicode classes:
    ...
    IsASCII          $code <= 127
    IsCntrl          m/^C/
    IsBlank          $code =~ m/^(0020|0009)$/ || m/^Z[^lp]/
    IsDigit          Nd
    IsGraph          m/^([LMNPS]|Co)/
    IsLower          Ll
    IsPrint          m/^([LMNPS]|Co|Zs)/
    IsPunct          m/^P/
    IsSpace          m/^Z/ || ($code =~ m/^(0009|000A|000B|000C|000D)$/
    IsSpacePerl      m/^Z/ || ($code =~ m/^(0009|000A|000C|000D|0085|2028|2029)$/
    IsUpper          m/^L[ut]/
    IsWord           m/^[LMN]/ || $code eq "005F"
    IsXDigit         $code =~ m/^00(3[0-9]|[46][1-6])$/
...
classes can only be used inside of a character class:
    ...
    m/\s+[abc[:digit:]xyz]\s*/;  # match a,b,c,x,y,z, or a digit
    m/^=item\s[[:digit:]]/;      # match '=item',
                                # followed by a space and a digit
    use charnames ":full";
    m/\s+[abc\p{IsDigit}xyz]\s+/;  # match a,b,c,x,y,z, or a digit
    m/^=item\s\p{IsDigit}/;        # match '=item',
                                  # followed by a space and a digit
...
A compiled regexp is some data structure
can be stored once and used again and again.  The regexp quote
...
Then C<$reg> can be used as a regexp:
    ...
    $x =~ m/$reg/;   # same thing, alternate form

C<$reg> can also be interpolated into a larger regexp:

    $x =~ m/(abc)?$reg/;  # still matches
...
algorithm.
    ...
    while( $command = <> ){
        ...
        if( ( @matches = $kwds =~ m/\b$command\w*/g ) == 1 ){
...
operation S<C<$kwds =~ m/\b($command\w*)/g>> does several things at the
...
a regexp using C<(?i)>, C<(?m)>, C<(?s)>, and C<(?x)>.  For instance,
    ...
    m/(?i)yes/;  # match 'yes' case insensitively
    m/yes/i;     # same thing
    m/(?x)(          # freeform version of an integer regexp
...
can be used to localize the modifier's effects:
    ...
    m/Answer: ((?i)yes)/;  # matches 'Answer: yes', 'Answer: YES', etc.
...
assertion is denoted by C<< (?<=fixed-regexp) >>.  Some examples are
    ...
    $x =~ m/cat(?=\s)/;   # matches 'cat' in 'housecat'
    @catwords = ($x =~ m/(?<=\s)cat\w+/g);  # matches,
                                           # $catwords[0] = 'catch'
                                           # $catwords[1] = 'catnip'
    $x =~ m/\bcat\b/;  # matches 'cat' in 'Tom-cat'
    $x =~ m/(?<=\s)cat(?=\s)/; # doesn't match; no isolated 'cat' in
                              # middle of $x
...
They evaluate true if the regexps do I<not> match:
    ...
    $x = "foobar";
    $x =~ m/foo(?!bar)/;  # doesn't match, 'bar' follows 'foo'
    $x =~ m/foo(?!baz)/;  # matches, 'baz' doesn't follow 'foo'
    $x =~ m/(?<!\s)foo/;  # matches, there is no \s before 'foo'
...
behavior occurs with completely independent regexps:
    ...
    $x = "ab";
    $x =~ m/a*/g;   # matches, eats an 'a'
    $x =~ m/\Gab/g; # doesn't match, no 'a' available
...
palindrome in between.
    ...
    m/(?: (\w) (?...Here be a palindrome...) \g{-1} | \w? )/x
...
in the regexp.  Here are some silly examples:
    ...
    $x = "abcdef";
    $x =~ m/abc(?{print "Hi Mom!";})def/; # matches,
                                         # prints 'Hi Mom!'
    $x =~ m/aaa(?{print "Hi Mom!";})def/; # doesn't match,
                                         # no 'Hi Mom!'

Pay careful attention to the next example:

    $x =~ m/abc(?{print "Hi Mom!";})ddd/; # doesn't match,
                                         # no 'Hi Mom!'
                                         # but why not?
...
example:
    ...
    $x =~ m/abc(?{print "Hi Mom!";})[d]dd/; # doesn't match,
                                           # but _does_ print
...
More fun with C<?{}>:
    ...
    $x =~ m/(?{print "Hi Mom!";})/;       # matches,
                                         # prints 'Hi Mom!'
    $x =~ m/(?{$c = 1;})(?{print "$c";})/;  # matches,
                                           # prints '1'
    $x =~ m/(?{$c = 1;})(?{print "$^R";})/; # matches,
                                           # prints '1'
...
a character got matched inside a group, we could use, e.g.,
    ...
    $x =~ m/(?{local $c = 0;})         # initialize count
           ( a                        # match 'a'
...
may surprise you:
    ...
    $bar = 5;
    $pat = '(?{ 1 })';
    m/foo(?{ $bar })bar/; # compiles ok, $bar not interpolated
    m/foo(?{ 1 })$bar/;   # compile error!
    m/foo${pat}bar/;      # compile error!

    $pat = qr/(?{ $foo = 1 })/;  # precompile code regexp
    m/foo${pat}bar/;      # compiles ok
...
plug it directly into a regexp:
    ...
    $text =~ m/$regexp/; # search $text for the $regexp
...
expression and matched immediately.  A simple example is
    ...
    $char = 'a';
    $x = 'aaaaabb';
    $x =~ m/(??{$char x $length})/x; # matches, there are 5 of 'a'
...
pragmas are
    ...
    use re 'taint';
    $tainted = <>;
    @parts = ($tainted =~ m/(\w+)\s+(\w+)/; # @parts is now tainted
...
the end of the block enclosing the pragmas.
    ...
    use re 'debug';
    m/^(.*)$/s;       # output debugging info

    use re 'debugcolor';
    m/^(.*)$/s;       # output debugging info in living color
...
termcap color sequences.  Here is example output:
    ...
    % perl -e 'use re "debug"; "abc" =~ m/a*b+c/;'
    Compiling REx `a*b+c'
    size 9 first at 1
       1: STAR(4)
       2:   EXACT <a>(0)
