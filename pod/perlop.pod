...
reference, Perl 5 interpreted as a filename pattern to be globbed, Perl
Kurila gives an error. This distinction is determined on syntactic
grounds alone.  That means C<< <$x> >> is always a readline() from
an indirect handle, but C<< <$hash{key}> >> gives an error.
That's because $x is a simple scalar variable, but C<$hash{key}> is
not--it's a hash element.  Even C<< <$x > >> (note the extra space)
gives an error. Use the C<glob> function to match filename patters.
