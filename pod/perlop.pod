...
pattern, or substitution.  The left argument is what is
supposed to be searched, or substituted instead of the default
...
If the right argument is an expression rather than a search pattern, or
substitution, it is interpreted as a search pattern at run
...
Binary ".." is the range operator, which can only be used in list context.
It returns a list of values counting (up by ones) from the left value to the right
...
Some examples of the range operator:

    for (101 .. 200) { print; }	# print $_ 100 times
    @foo = @foo[0 .. $#foo];	# an expensive no-op
    @foo = @foo[$#foo-4 .. $#foo];	# slice last 5 items

The range operator makes use of the magical
...
X<\t> X<\n> X<\r> X<\f> X<\b> X<\a> X<\e> X<\x> X<\0> X<\c> X<\N> X<\l> X<\u> X<\L> X<\U> X<\E> X<\Q>
...
or "C<@>" or blocks beggining with "C<{>" are interpolated.  Subscripted variables such as C<$a[3]> or
...
The result may be used as a subpattern in a match:
    ...
    $re = qr/$pattern/;
    $string =~ m/foo${re}bar/;	# can be interpolated in other patterns
    $string =~ $re;		# or used standalone
    $string =~ m/$re/;		# or this way
...
time a match C<m/$pat/> is attempted.  (Perl has many other internal
...
Examples:
    ...
    open(TTY, '/dev/tty');
    <TTY> =~ m/^y/i && foo();	# do foo if desired
    ...
    $arg = shift;
    while (~< *ARGV) {
	print if m/$arg/o;	# compile only once
    }

    if (($F1, $F2, $Etc) = ($foo =~ m/^(\S+)\s+(\S+)\s*(.*)/))
...
Examples:
    ...
    # list context
    ($one,$five,$fifteen) = (`uptime` =~ m/(\d+\.\d+)/g);

    # scalar context
    $/ = "";
    while (defined($paragraph = ~< *ARGV)) {
	while ($paragraph =~ m/[a-z]['")]*[.!?]+['")]*\s/g) {
	    $sentences++;
	}
    }
    print "$sentences\n";
    ...
    while ($i++ +< 2) {
        print "1: '";
        print $1 while m/(o)/gc; print "', pos=", pos, "\n";
        print "2: '";
        print $1 if m/\G(q)/gc;  print "', pos=", pos, "\n";
        print "3: '";
        print $1 while m/(p)/gc; print "', pos=", pos, "\n";
    }
...
A useful idiom for C<lex>-like scanners is C<m/\G.../gc>.  You can
...
regexp tries to match where the previous one leaves off.
 ...
 LOOP:
    {
      print(" digits"),		redo LOOP if m/\G\d+\b[,.;]?\s*/gc;
      print(" lowercase"),	redo LOOP if m/\G[a-z]+\b[,.;]?\s*/gc;
      print(" UPPERCASE"),	redo LOOP if m/\G[A-Z]+\b[,.;]?\s*/gc;
      print(" Capitalized"),	redo LOOP if m/\G[A-Z][a-z]+\b[,.;]?\s*/gc;
      print(" MiXeD"),		redo LOOP if m/\G[A-Za-z]+\b[,.;]?\s*/gc;
      print(" alphanumeric"),	redo LOOP if m/\G[A-Za-z0-9]+\b[,.;]?\s*/gc;
      print(" line-noise"),	redo LOOP if m/\G[^A-Za-z0-9]+/gc;
      print ". That's all!\n";
    }
...
=item s/PATTERN/REPLACEMENT/msixpogc
X<substitute> X<substitution> X<replace> X<regexp, replace>
X<regexp, substitute> X</m> X</s> X</i> X</x> X</p> X</o> X</g> X</c>
...
For constructs with three-part delimiters (C<s///>), the search is repeated once more.
If the first delimiter is not an opening punctuation, three delimiters must
be same such as C<s!!!> and C<s)))>, in which case the second delimiter
...
delimiters such as C<s(){}> and C<s[]//>.  In these cases, whitespaces
...
No interpolation is performed. 
Note that the combination C<\\> is left intact, as is C<\'>.

=item C<"">, C<``>, C<qq//>, C<qx//>, C<<<"EOF">
...
It is at this stage that C<split()> silently optimizes C<m/^/> to
mean C<m/^/m>.
...
The ARGV filehandle is special: it can be used to emulate the
behavior of B<sed> and B<awk>.  Input from ~< *ARGV comes either from
standard input, or from each file listed on the command line.  Here's
how it works: the first time ~< *ARGV is evaluated, the @ARGV array is
...
You can modify @ARGV before the first ~< *ARGV as long as the array ends up
