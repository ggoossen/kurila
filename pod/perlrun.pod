...
B<perl>	S<[ B<TtuUWX> ]>
	...
	S<[ B<-pna> ] [ B<-l>[I<octal>] ] [ B<-0>[I<octal/hexadecimal>] ]>
...
clustered with the following switch, if any.
    ...
    #!/usr/bin/perl -pi.orig	# same as -p -i.orig
...
It can for instance be used to add entries to the $^INCLUDE_PATH array to make perl
...
logical device for the $^INCLUDE_PATH path on VMS only.  Other logical names that
