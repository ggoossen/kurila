...
C<rindex>, C<sprintf>, C<substr>, C<uc>, C<ucfirst>
...
C<binmode>, C<close>, C<closedir>, C<die>, C<eof>,
...
C<scalar>, C<state>, C<undef>, C<wantarray>
...
C<bless>, C<package>, C<ref>, C<tie>, C<tied>,
C<use>
...
C<dump>, C<endgrent>, C<endhostent>,
...
to go back before the current one.
    ...
    #  0         1          2      3            4
    ($package, $filename, $line, $subroutine, $args,
...
C<$args> is has the value of C<@_> of the frame or C<undef> if the frame has no C<@_>.
...
See L<kurilasyn> for alternative strategies.
...
filename for error messages, searches the $^INCLUDE_PATH directories, and updates
C<$^INCLUDED> if the file is found.  See L<perlvar/Predefined Names> for these
...
If MODE is C<< '<' >> or nothing, the file
...
The MODE argument may be used to specify IO "layers"
...
You may also, in the Bourne shell tradition, specify a MODE beginning
with C<< '>&' >>, in which case the FILENAME should be a reference to a glob 
or a name of a filehandle (or file descriptor, if numeric) to be
...
If you open a pipe on the command C<'-'>, then
...
of values, as follows:
    ...
    C	An unsigned char (octet) value, single byte.
    W   An Unicode codepoint (can be greater than 255).
...
Demands some semantics specified by EXPR or by C<$_> if EXPR is not supplied.

C<require> demands that a library file be included if it
...
to the included code.  Has semantics similar to the following subroutine:
    ...
    sub require {
       ...
       ITER: {
           ...
           die "Can't find $filename in \$^INCLUDE_PATH";
       }
       if ($@) {
           $INC{$filename} = undef;
           die $@;
...
directories specified in the C<$^INCLUDE_PATH> array.
...
The require function will look for the "F<Foo::Bar>" file in the $^INCLUDE_PATH array and
...
Perl code into the $^INCLUDE_PATH array.  There are three forms of hooks: subroutine
references, array references and blessed objects.

Subroutine references are the simplest case.  When the inclusion system
walks through $^INCLUDE_PATH and encounters a subroutine, this subroutine gets
...
is returned then C<require> will look at the remaining elements of $^INCLUDE_PATH.
...
into package C<main>.)  Here is a typical code layout:
    ...
    # In the main program
    push $^INCLUDE_PATH, Foo->new(...);

Note that these hooks are also permitted to set the $^INCLUDED entry
corresponding to the files they have loaded. See L<perlvar/$^INCLUDED>.
...
loop control operators described in L<kurilasyn> or with C<goto>.
...
array, and returns the new number of elements in the array.
    ...
    unshift(@ARGV, '-e') unless $ARGV[0] =~ m/^-/;
