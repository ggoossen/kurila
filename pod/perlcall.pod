...
type of event, specify the G_EVAL flag.  It will put an I<try { }>
around the subroutine call.

Whenever control returns from the I<call_*> function you need to
check the C<ERRSV> variable as you would in a normal Perl script.
The error can be rethrown using C<die_where(ERRSV)>.
