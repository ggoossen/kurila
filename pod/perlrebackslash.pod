...
=head4 Example
 ...
 $str =~ m/\t/;   # Matches if $str contains a (horizontal) tab.
...
=head4 Example
 ...
 $str =~ m/\cK/;  # Matches if $str contains a vertical tab (control-K).
...
=head4 Example
 ...
 $str =~ m/\N{THAI CHARACTER SO SO}/;  # Matches the Thai SO SO character

 use charnames 'Cyrillic';            # Loads Cyrillic names.
 $str =~ m/\N{ZHE}\N{KA}/;             # Match "ZHE" followed by "KA".
...
=head4 Examples
 ...
 $str = "Perl";
 $str =~ m/\120/;    # Match, "\120" is "P".
 $str =~ m/\120+/;   # Match, "\120" is "P", it is repeated at least once.
 $str =~ m/P\053/;   # No match, "\053" is "+" and taken literally.
...
=head4 Examples
 ...
 $str = "Perl";
 $str =~ m/\x50/;    # Match, "\x50" is "P".
 $str =~ m/\x50+/;   # Match, "\x50" is "P", it is repeated at least once.
 $str =~ m/P\x2B/;   # No match, "\x2B" is "+" and taken literally.
...
=head4 Examples
 ...
 $greg    = "GrEg";
 $miranda = "(Miranda)";
 $str     =~ m/\u$sid/;        # Matches 'Sid'
 $str     =~ m/\L$greg/;       # Matches 'greg'
 $str     =~ m/\Q$miranda\E/;  # Matches '(Miranda)', as if the pattern
...
=head4 Examples
  ...
  "cat"   =~ m/\bcat\b/;   # Matches.
  "cats"  =~ m/\bcat\b/;   # No match.
  "cat"   =~ m/\bcat\B/;   # No match.
  "cats"  =~ m/\bcat\B/;   # Match.

  while ("cat dog" =~ m/(\w+)/g) {
      print $1;           # Prints 'catdog'
  }
  while ("cat dog" =~ m/\G(\w+)/g) {
      print $1;           # Prints 'cat'
  }
...
=head4 Examples
 ...
 "\x{256}" =~ m/^\C\C$/;    # Match as chr (256) takes 2 octets in UTF-8.

 $str =~ s/foo\Kbar/baz/g; # Change any 'bar' following a 'foo' to 'baz'.
 $str =~ s/(.)\K\1//g;     # Delete duplicated characters.

 "\n"   =~ m/^\R$/;         # Match, \n   is a generic newline.
 "\r"   =~ m/^\R$/;         # Match, \r   is a generic newline.
 "\r\n" =~ m/^\R$/;         # Match, \r\n is a generic newline.

 "P\x{0307}" =~ m/^\X$/     # \X matches a P with a dot above.
