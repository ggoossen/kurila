...
and closing parentheses and brackets."
  ...
  (save-excursion
    (let ((i (kurila-sniff-for-indent parse-data)) what p)
      (cond
       ;;((or (null i) (eq i t) (numberp i))
       ;;  i)
       ((vectorp i)
	(setq what (assoc (elt i 0) kurila-indent-rules-alist))
	(cond
	 ...
	 ((eq 'indentable (elt i 0))	; Indenter for REGEXP qw() etc
	  (cond		       ;;; [indentable terminator start-pos is-block]
	   ...
	   ((eq 'first-line (elt i 1)); [indentable first-line start-pos]
	    (goto-char (elt i 2))
	    (+ (or kurila-regexp-indent-step kurila-indent-level)
	       -1
	       (current-column)))
	   ((eq 'cont-line (elt i 1)); [indentable cont-line pos prev-pos first-char start-pos]
	    ...
	    (goto-char (elt i 3))	; previous line
	    (+ (* p (or kurila-regexp-indent-step kurila-indent-level))
	       (cond ((eq what ?\) )
		      (- kurila-close-paren-offset)) ; compensate
		     ((eq what ?\| )
		      (- (or kurila-regexp-indent-step kurila-indent-level)))
		     (t 0))
	 ...
	 ((eq 'toplevel (elt i 0)) ;; [toplevel start char-after state immed-after-block]
	  (+ (save-excursion		; To beg-of-defun, or end of last sexp
	       ...
	       (- (current-indentation) ; 
		  (if (elt i 4) kurila-indent-level 0)))	; immed-after-block
	     (if (eq (elt i 2) ?{) kurila-continued-brace-offset 0) ; char-after
	     ;; Look at previous line that's at column 0
	     ;; to determine whether we are in top-level decls
	     ;; or function's arg decls.  Set basic-indent accordingly.
	     ;; Now add a little if this is a continuation line.
	 ...
	 ((eq 'code-start-in-block (elt i 0))
	  ...
	  ;; start there too.  If kurila-indent-level=0,
