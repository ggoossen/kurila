...
my $arg = env::var('PATH'); # a tainted value
...
for my $ary (@(\@(ascii => 'perl'), \@(latin1 => "\x[B6]"), \@(utf8 => "\x{100}"))) {
    ...
    my $taint = $arg; substr($taint, 0, undef, $utf8);
    utf8::encode($taint);
...
do {
    fresh_perl_is('use utf8; $a = substr $^EXECUTABLE_NAME, 0, 0; m/\x{100}/i; m/$a\x{100}/i || print q,ok,',
		  'ok', \%(switches => \@("-T", "-l")),
		  "matching a regexp is taint agnostic");

    fresh_perl_is('use utf8; $a = substr $^EXECUTABLE_NAME, 0, 0; m/$a\x{100}/i || print q,ok,',
		  'ok', \%(switches => \@("-T", "-l")),
