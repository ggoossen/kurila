...
do
    ...
    if ($Cwd::VERSION)  # Avoid a 'used only once' warning
        local $^WARNING = 0
        *rel2abs = sub ($self, $path, ?$base)
            local $^WARNING = 0
            local *Cwd::getdcwd = sub (?$drive)
                return 'D:\alpha\beta' if $drive eq 'D:'
                return 'C:\one\two'    if $drive eq 'C:'
                return
            
            *Cwd::getdcwd = *Cwd::getdcwd # Avoid a 'used only once' warning
            return $self->SUPER::rel2abs: $path, $base
...
#
sub tryfunc($function, $expected, ?$platform)
    :SKIP do
