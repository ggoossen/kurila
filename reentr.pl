...
?RCS:
?MAKE:d_$func_r $func_r_proto: @prereq
?MAKE:	-pick add \$@ \%<
?S:d_$func_r:
?S:	This variable conditionally defines the HAS_$FUNC_R symbol,
?S:	which indicates to the C program that the $func_r()
?S:	routine is available.
?S:.
?S:$func_r_proto:
?S:	This variable encodes the prototype of $func_r.
?S:	It is zero if d_$func_r is undef, and one of the
?S:	REENTRANT_PROTO_T_ABC macros of reentr.h if d_$func_r
?S:	is defined.
?S:.
?C:HAS_$FUNC_R:
?C:	This symbol, if defined, indicates that the $func_r routine
?C:	is available to $func re-entrantly.
?C:.
?C:$FUNC_R_PROTO:
?C:	This symbol encodes the prototype of $func_r.
?C:	It is zero if d_$func_r is undef, and one of the
?C:	REENTRANT_PROTO_T_ABC macros of reentr.h if d_$func_r
?C:	is defined.
?C:.
?H:#\$d_$func_r HAS_$FUNC_R	   /**/
?H:#define $FUNC_R_PROTO \$$func_r_proto	   /**/
?H:.
?T:try hdrs d_$func_r_proto
?LINT:set d_$func_r
?LINT:set $func_r_proto
: see if $func_r exists
set $func_r d_$func_r
eval \$inlibc
case "\$d_$func_r" in
"\$define")
EOF
	print <<EOF;
	hdrs="$hdrs"
	case "\$d_$func_r_proto:\$usethreads" in
	":define")	d_$func_r_proto=define
		set d_$func_r_proto $func_r \$hdrs
		eval \$hasproto ;;
	*)	;;
	esac
	case "\$d_$func_r_proto" in
	define)
EOF
    }
    for my $p (@p) {
	...
	if (%opts{U}) {
	    print <<EOF ;
	case "\$$func_r_proto" in
	''|0) try='%m{$r} $func_r($v);'
	./protochk "extern \$try" \$hdrs && $func_r_proto=$p ;;
	esac
EOF
        }
        %seenh{$func}->{$p}++;
	...
	''|0)	d_$func_r=undef
 	        $func_r_proto=0
		echo "Disabling $func_r, cannot determine prototype." >&4 ;;
	* )	case "\$$func_r_proto" in
		REENTRANT_PROTO*) ;;
		*) $func_r_proto="REENTRANT_PROTO_\$$func_r_proto" ;;
		esac
		echo "Prototype: \$try" ;;
	esac
	;;
	*)	case "\$usethreads" in
		define) echo "$func_r has no prototype, not using it." >&4 ;;
		esac
		d_$func_r=undef
		$func_r_proto=0
		;;
...
sub pushinitfree {
    my $func = shift;
    push @init, <<EOF;
	Newx(PL_reentrant_buffer->_$func_buffer, PL_reentrant_buffer->_$func_size, char);
EOF
    push @free, <<EOF;
	Safefree(PL_reentrant_buffer->_$func_buffer);
EOF
}
...
EOF
    ...
    push @define, <<EOF;
#   define USE_$GENFUNC_$n
#else
#   undef  USE_$GENFUNC_$n
#endif
...
EOF
	    push @size, <<EOF;
	PL_reentrant_buffer->_$func_size = REENTRANTSMALLSIZE;
EOF
	    pushinitfree $func;
	    pushssif $endif;
	}
	...
	%seend{$func} _$func_data;
#else
	%seent{$func} *_$func_struct_buffer;
#endif
EOF
    	    push @init, <<EOF;
#if CRYPT_R_PROTO != REENTRANT_PROTO_B_CCD
	PL_reentrant_buffer->_$func_struct_buffer = 0;
#endif
EOF
    	    push @free, <<EOF;
#if CRYPT_R_PROTO != REENTRANT_PROTO_B_CCD
	Safefree(PL_reentrant_buffer->_$func_struct_buffer);
#endif
EOF
	    pushssif $endif;
	}
...
EOF
	    ...
	    push @struct, <<EOF;
	%seent{$func}	_$genfunc_struct;
	char*	_$genfunc_buffer;
	size_t	_$genfunc_size;
EOF
            push @struct, <<EOF;
#   ifdef USE_$GENFUNC_PTR
	%seent{$func}*	_$genfunc_ptr;
#   endif
EOF
	    push @struct, <<EOF;
#   ifdef USE_$GENFUNC_FPTR
	FILE*	_$genfunc_fptr;
#   endif
EOF
	    push @init, <<EOF;
#   ifdef USE_$GENFUNC_FPTR
	PL_reentrant_buffer->_$genfunc_fptr = NULL;
#   endif
EOF
...
EOF
    	    push @struct, <<EOF;
#   ifdef USE_$GENFUNC_ERRNO
	int	_$genfunc_errno;
#   endif 
EOF
	    push @size, <<EOF;
#if   !($D)
	PL_reentrant_buffer->_$genfunc_size = REENTRANTUSUALSIZE;
#endif
EOF
	    push @init, <<EOF;
#if   !($D)
	Newx(PL_reentrant_buffer->_$genfunc_buffer, PL_reentrant_buffer->_$genfunc_size, char);
#endif
EOF
	    push @free, <<EOF;
#if   !($D)
	Safefree(PL_reentrant_buffer->_$genfunc_buffer);
#endif
EOF
	    pushssif $endif;
	}
	...
	size_t	_$func_size;
#   if $R
	%seent{$func}*	_$func_ptr;
#   endif
EOF
	    push @size, <<EOF;
	/* This is the size Solaris recommends.
	 * (though we go static, should use pathconf() instead) */
	PL_reentrant_buffer->_$func_size = sizeof(%seent{$func}) + MAXPATHLEN + 1;
EOF
    	    push @init, <<EOF;
	PL_reentrant_buffer->_$func_struct = (%seent{$func}*)safemalloc(PL_reentrant_buffer->_$func_size);
EOF
	    push @free, <<EOF;
	Safefree(PL_reentrant_buffer->_$func_struct);
EOF
	    pushssif $endif;
	}
...
EOF
	...
	for my $p (@p) {
	    ...
	    push @wrap, <<EOF;
#   if !defined($func) && $FUNC_R_PROTO == REENTRANT_PROTO_$p
EOF
	    if ($r eq 'V' || $r eq 'B') {
	        push @wrap, <<EOF;
#       define $func($v) $call
