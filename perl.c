...
#  endif	       
    ...
    av_push(res, newSVpv(PL_bincompat_options, 0));
    av_push(res, Perl_newSVpvn_flags(aTHX_ non_bincompat_options,
	    sizeof(non_bincompat_options) - 1, 0));

#ifdef __DATE__
#  ifdef __TIME__
    av_push(res, Perl_newSVpvn_flags(aTHX_
			      STR_WITH_LEN("Compiled at " __DATE__ " " __TIME__),
			      0));
#  else
    av_push(res, Perl_newSVpvn_flags(aTHX_ STR_WITH_LEN("Compiled on " __DATE__),
			      0));
#  endif
#else
    ...
    av_push(res, Perl_newSVpvn_flags(aTHX_ STR_WITH_LEN(OSNAME), 0));

    patches = newAV();
    av_push(res, patches);
    for (i = 1; i <= local_patch_count; i++) {
	/* This will be an undef, if PL_localpatches[i] is NULL.  */
	av_push(patches, newSVpv(PL_localpatches[i], 0));
    }
...
Perl_get_debug_opts(pTHX_ const char **s, bool givehelp)
{
	...
	static const char debopts[] = "psltocPmfrxuUHXDSTRJvCAqB";
